...
# registry here - that pulls in the analysis package -> app_helper ->
# Flask machinery, which is not safe at config import time. The dim
# lookup table below mirrors the ``embedding_dim`` class attributes on
# the shipped backends; adding a new backend means adding a row here too.
